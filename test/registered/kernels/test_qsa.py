...
def test_qsa_chunk_prefill_packs_history_into_geometric_scratch():
    backend = QwenSparseAttnBackend.__new__(QwenSparseAttnBackend)
    backend._fa2_scratch = {}
    req_to_token = torch.tensor(
        [[3, 1, 4, 2, 0, 5], [11, 8, 10, 7, 9, 6]], dtype=torch.int32
    )
    k_buffer = torch.arange(12 * 2, dtype=torch.float32).reshape(12, 1, 2)
    v_buffer = k_buffer + 1000

    packed_k, packed_v = backend._pack_context_kv(
        k_buffer, v_buffer, req_to_token, [0, 1], [3, 2]
    )
    expected_slots = torch.tensor([3, 1, 4, 11, 8])
    ...
    assert backend._fa2_scratch[key][0] is first_buffers[0]
    torch.testing.assert_close(smaller_k, k_buffer[torch.tensor([3, 1, 4, 2])])

    backend._pack_context_kv(k_buffer, v_buffer, req_to_token, [0, 1], [6, 6])
